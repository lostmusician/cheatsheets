...
Represents programs, languages, compilers, and interpreters. Used to reason about portability and execution. A program $P$ implemented in $A$ can run on machine $C$ if there is a chain of interpreters/compilers mapping $A \to C$.
...
\texttt{byte <: short <: int <: long <: float <: double}\\
\texttt{char <: int}
...
\begin{itemize}
  ...
  \item Useful when consumer does not know exact subtype; acts as auxiliary
  \item Static method
  \item Can capture generic types to correctly parameterise
\end{itemize}

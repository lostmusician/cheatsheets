...
\noindent
{\footnotesize
\begin{tabular}{|l|>{\raggedright\arraybackslash}p{2.6cm}|>{\raggedright\arraybackslash}p{2.9cm}|}
\hline
\textbf{Feature} & \textbf{Overloading (Static)} & \textbf{Overriding (Dynamic)} \\ \hline
\textbf{Scope} & Same class or inherited & Between subclass \& superclass \\ \hline
\textbf{Signature} & \textbf{Different} params (type, order, \#) & \textbf{Same} params \& name \\ \hline
\textbf{Return Type} & Can be different & \textbf{Covariant} ($Ret_{sub} <: Ret_{parent}$) \\ \hline
\textbf{Access} & Can be different & \textbf{Non-weaker} (same or more open) \\ \hline
\textbf{Exceptions} & Can be different & \textbf{Fewer} or \textbf{more specific} checked \\ \hline
\textbf{Binding} & \textbf{Compile-Time} (CTT of args) & \textbf{Run-Time} (RTT of target) \\ \hline
\end{tabular}
}
...
\section{Type Casting \& Checking}
\texttt{a = (C) b;} where \texttt{b} has \texttt{CTT(b)} and \texttt{RTT(b)}.

\subsection{Compile-Time Check (The ``Is it Possible?'' Rule)}
The compiler allows the cast if there is any theoretical chance \texttt{RTT(b) <: C}.
\begin{itemize}
    \item \textbf{Widening}: \texttt{CTT(b) <: C}. Always allowed; no runtime check needed.
    \item \textbf{Narrowing}: \texttt{C <: CTT(b)}. Allowed; requires runtime check (\texttt{ClassCastException} if fails).
    \item \textbf{Interface Exception}: If \texttt{C} is an interface and \texttt{CTT(b)} is a non-final class, the cast is \textit{always} allowed at compile-time. Reason: A future subclass of \texttt{CTT(b)} could implement \texttt{C}.
    \item \textbf{Cross-Casting}: Casting between two unrelated interfaces is always allowed at compile-time for the same reason (a class could implement both).
    \item \textbf{Hard Fail}: If \texttt{C} and \texttt{CTT(b)} are unrelated classes, OR \texttt{CTT(b)} is \texttt{final} and does not implement interface \texttt{C}.
\end{itemize}

\subsection{Run-Time Check}
The JVM ignores \texttt{CTT} and looks only at the heap:
\begin{itemize}
    \item Check: \texttt{RTT(b) <: C}? 
    \item \textbf{Success}: Execution continues; \texttt{CTT(a)} is now \texttt{C}.
    \item \textbf{Failure}: Throws \texttt{ClassCastException}.
\end{itemize}
